...
impl PyProjectInfo {
    ...
    fn from_string(contents: &String) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let has_project_dep = parsed.get("project")
            .and_then(|t| t.get("dependencies"))
            .is_some();

        let has_project_dep_optional = parsed.get("project")
        .and_then(|t| t.get("optional-dependencies"))
        .is_some();

        let has_poetry_dep = parsed.get("tool")
            .and_then(|t| t.get("poetry"))
            .and_then(|t| t.get("dependencies"))
            .is_some();

        let has_poetry_dep_group = parsed.get("tool")
        .and_then(|t| t.get("poetry"))
        .and_then(|p| p.get("group"))
        .and_then(|groups| groups.as_table())
        .map_or(false, |groups| {
            groups.values().any(|group| group.get("dependencies").is_some())
        });
